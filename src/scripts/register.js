...
function applyLanguage(newLanguage) {
	...
	const traduction = i18n[language];
	document.querySelectorAll('[data-i18n]').forEach(element => {
			const key = element.dataset.i18n;
			if (traduction[key] !== undefined) element.textContent = traduction[key];
	});
...
function updateDots(passwordLength) {
	...
	const companyState = document.getElementById('empresa').value.trim() !== '';
...
['empresa', 'nombre', 'apellido', 'email'].forEach(id =>
...
document.getElementById('registerForm').addEventListener('submit', async (event) => {
	...
	if (!empresa || !nombre || !apellido || !email || !password) return showError(traduction.err_required);
	if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return showError(traduction.err_email);
	if (password.length < 8) return showError(traduction.err_short_pw);
	...
	try {
		...
		const res = await fetch('../api/auth/register.php', {
			...
			body: JSON.stringify({empresa, nombre, apellido, email, password}),
		});
		...
		if (data.success) {
			...
			const welcomeMsg = language === 'es'
				? `¡Bienvenido, ${data.user.nombre}! Tu cuenta ha sido creada.`
				: `Welcome, ${data.user.nombre}! Your account has been created.`;
			...
			setTimeout(() => {
				window.location.href = routes[data.user.rol] ?? '../views/dashboard.html';
			}, 1800);

		} 
		else {
			showError(data.error || traduction.err_server);
			btn.classList.remove('loading');
			btn.disabled = false;
		}
	} catch {
		showError(traduction.err_network);
		btn.classList.remove('loading');
		btn.disabled = false;
	}
});
