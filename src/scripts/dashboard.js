...
async function apiFetch(modulo, method = 'GET', body = null, id = null) {
  try {
    const key = MODULE_MAP[modulo] ?? modulo;
    let url = `${API}${DB}_${key}`;
    if (id) url += `&id=${id}`;
    const opts = { method, headers: AUTH_HEADERS };
...
/* ── PK por módulo ─────────────────────────────────────────────── */
const PK = {
  proyectos:'id_proyecto', contratos:'id_contrato', licitaciones:'id_licitacion',
  hitos:'id_hito', bitacora:'id_bitacora', documentos:'id_documento',
  riesgos:'id_riesgo', permisos:'id_permiso',
  empleados:'id_empleado', departamentos:'id_departamento',
  asignaciones:'id_asignacion', ausencias:'id_ausencia',
  capacitaciones:'id_capacitacion', emp_cap:'id_emp_cap',
  accidentes:'id_accidente', nomina:'id_nomina',
  detalle_nomina:'id_detalle_nomina', prestamos:'id_prestamo',
  facturas:'id_factura', pagos:'id_pago', retenciones:'id_retencion',
  cotizaciones:'id_cotizacion', garantias:'id_garantia',
  proveedores:'id_proveedor', contactos:'id_contacto',
  subcontratistas:'id_subcontratista', contratos_sub:'id_contrato_sub',
  equipos:'id_equipo', mantenimiento:'id_mantenimiento',
  almacenes:'id_almacen', clientes:'id_cliente', seguridad:'id_incidente',
};

/* ── Navegación ────────────────────────────────────────────────── */
const PAGES = {
  dashboard:'Panel General', proyectos:'Proyectos', contratos:'Contratos',
  licitaciones:'Licitaciones', hitos:'Hitos', bitacora:'Bitácora de Obra',
  documentos:'Documentos', riesgos:'Riesgos', permisos:'Permisos / Licencias',
  empleados:'Empleados', departamentos:'Departamentos', asignaciones:'Asignaciones',
  ausencias:'Ausencias', capacitaciones:'Capacitaciones', emp_cap:'Registro Capacitación',
  accidentes:'Accidentes Laborales', nomina:'Nómina', detalle_nomina:'Detalle de Nómina',
  prestamos:'Préstamos',
  facturas:'Facturas', pagos:'Pagos', retenciones:'Retenciones',
  cotizaciones:'Cotizaciones', garantias:'Garantías',
  proveedores:'Proveedores', contactos:'Contactos Prov./Sub.',
  subcontratistas:'Subcontratistas', contratos_sub:'Contratos Sub.',
  equipos:'Equipos', mantenimiento:'Mantenimiento', almacenes:'Almacenes',
  clientes:'Clientes', seguridad:'Incidentes de Seguridad', tienda:'Tienda',
};

const SQL_MODULES = Object.keys(PAGES).filter(k => k !== 'dashboard' && k !== 'tienda');
...
/* Texto visible en cada select por tipo */
...
function pkFn(tipo, r) {
  const map = {
    proyectos:'id_proyecto', clientes:'id_cliente', empleados:'id_empleado',
    proveedores:'id_proveedor', subcontratistas:'id_subcontratista',
    contratos:'id_contrato', licitaciones:'id_licitacion',
    departamentos:'id_departamento', capacitaciones:'id_capacitacion',
    nomina:'id_nomina', facturas:'id_factura', equipos:'id_equipo',
  };
...
const PLACEHOLDER = {
  proyectos:'— Seleccionar proyecto —', clientes:'— Seleccionar cliente —',
  empleados:'— Seleccionar empleado —', proveedores:'— Seleccionar proveedor —',
  subcontratistas:'— Seleccionar subcontratista —', contratos:'— Seleccionar contrato —',
  licitaciones:'— Seleccionar licitación —', departamentos:'— Seleccionar departamento —',
  capacitaciones:'— Seleccionar capacitación —', nomina:'— Seleccionar período —',
  facturas:'— Seleccionar factura —', equipos:'— Seleccionar equipo —',
};
...
function toggleSidebar() {
  ...
  if (isMobile) {
    /* Móvil: comportamiento original — slide sobre el contenido */
    sidebar.classList.toggle('open');
    overlay.classList.toggle('open');
  } else {
    /* Desktop: colapsar/expandir empujando el contenido */
...
function closeSidebar() {
  ...
  /* En desktop no cerramos al navegar, solo el botón hamburger lo hace */
}
...
async function submitForm(module) {
  ...
  if (result && result.error) { toast('Error: ' + result.error, 'error'); }
  else { toast('Registro guardado exitosamente.'); form.reset(); await cargarTabla(module); await cargarDashboard(); }
  return false;
}
...
async function deleteRecord(module, id) {
  ...
  if (result && result.error) { toast('Error al eliminar: ' + result.error, 'error'); }
  else { toast('Registro eliminado.', 'error'); await cargarTabla(module); await cargarDashboard(); }
}

/* ── Formato ───────────────────────────────────────────────────── */
function fmtMXN(v) { return parseFloat(v||0).toLocaleString('es-MX', {style:'currency', currency:'MXN'}); }
function badgeStatus(val) {
  if (!val) return '<span class="badge badge-gray">—</span>';
  const v = val.toString();
  const map = {
    Activo:'badge-green', Activa:'badge-green', Vigente:'badge-green', Pagada:'badge-green',
    Aprobada:'badge-green', Completado:'badge-green', Ganada:'badge-green', Liquidado:'badge-green',
    Cumplido:'badge-green', Firmado:'badge-green', En_Ejecucion:'badge-blue', Presentada:'badge-blue',
    ...
    Inactivo:'badge-red', Vencida:'badge-red', Cancelado:'badge-red', Cancelada:'badge-red',
    Baja:'badge-red', Perdida:'badge-red', Materializado:'badge-red', Ejecutada:'badge-red',
    Revocado:'badge-red', Rechazada:'badge-red', Cerrado:'badge-red',
    Pausado:'badge-gold', En_Riesgo:'badge-gold', Por_Vencer:'badge-gold', Mitigado:'badge-gold',
...
function renderTable(module, datos) {
  ...
  data.forEach(r => {
    ...
    switch (module) {
      ...
      case 'emp_cap':
        html += `<tr><td>${id}</td><td>${r.id_empleado||'—'}</td><td>${r.id_capacitacion||'—'}</td><td>${r.fecha_asistencia||'—'}</td><td>${r.calificacion||'—'}</td><td>${r.aprobado=='1'?'<span class="badge badge-green">Sí</span>':r.aprobado=='0'?'<span class="badge badge-red">No</span>':'—'}</td><td>${r.certificado||'—'}</td><td><div class="td-actions">${edit}${del}</div></td></tr>`; break;
      case 'accidentes':
...
html += `<tr><td>${id}</td><td>${r.id_subcontratista||'—'}</td><td>${r.numero_contrato}</td><td>${badgeStatus(r.estado)}</td><td>${fmtMXN(r.monto_contratado)}</td><td>${r.fecha_inicio||'—'}</td><td>${r.fecha_fin_est||'—'}</td><td style="max-width:180px;overflow:hidden;text-overflow:ellipsis">${r.descripcion_trabajo||'—'}</td><td><div class="td-actions">${edit}${del}</div></td></tr>`; break;      case 'equipos':
      ...
      case 'seguridad':
        html += `<tr><td>${id}</td><td>${r.tipo_incidente||'—'}</td><td>${badgeStatus(r.estado)}</td><td>${r.fecha_hora||'—'}</td><td>${fmtMXN(r.costo_incidente)}</td><td>${r.archivo||'—'}</td><td style="max-width:200px;overflow:hidden;text-overflow:ellipsis">${r.descripcion||'—'}</td><td><div class="td-actions">${edit}${del}</div></td></tr>`; break;
      default:
...
const UPDATE_FIELDS = {
  ...
  emp_cap:         ['fecha_asistencia','calificacion','aprobado','certificado'],
  ...
  clientes:        ['tipo_persona','nombre_razon_social','nombre_contacto','telefono','email','direccion','ciudad','estado','pais','codigo_postal','comprobante_domicilio','calificacion','notas','activo'],
...
const FIELD_META = {
  ...
  /* fechas */
  fecha_inicio:           { type:'date' }, fecha_fin:              { type:'date' },
  fecha_estimada:         { type:'date' }, fecha_real:             { type:'date' },
  fecha_pago:             { type:'date' }, fecha_vencimiento:      { type:'date' },
  fecha_retencion:        { type:'date' }, fecha_liberacion:       { type:'date' },
  fecha_liquidacion_est:  { type:'date' }, fecha_fin_contrato:     { type:'date' },
  fecha_fin_estimada:     { type:'date' }, fecha_fin_real:         { type:'date' },
  fecha_fin_est:          { type:'date' }, proxima_revision:       { type:'date' },
  fecha_inicio_estimada:  { type:'date' }, fecha_inicio_real:      { type:'date' },
  fecha_cierre:           { type:'date' }, fecha_fallo:            { type:'date' },
  /* numéricos */
  salario_base:        { type:'number' }, monto_total:        { type:'number' },
  monto:               { type:'number' }, costo:              { type:'number' },
  costo_hora:          { type:'number' }, costo_estimado:     { type:'number' },
  costo_incidente:     { type:'number' }, total_cotizado:     { type:'number' },
  monto_cobro:         { type:'number' }, presupuesto_estimado:{ type:'number' },
  costo_actual:        { type:'number' }, penalizacion_dia:   { type:'number' },
  anticipo_pct:        { type:'number' }, porcentaje_avance:  { type:'number' },
  porcentaje:          { type:'number' }, calificacion:       { type:'number' },
  horas_semanales:     { type:'number' }, dias_habiles:       { type:'number' },
  dias_incapacidad:    { type:'number' }, duracion_horas:     { type:'number' },
  numero_pagos:        { type:'number' }, monto_pendiente:    { type:'number' },
  monto_liberado:      { type:'number' }, monto_contratado:   { type:'number' },
  monto_descuento_quincenal: { type:'number' },
  total_percepciones:  { type:'number' }, total_deducciones:  { type:'number' },
  total_neto:          { type:'number' }, neto_pagar:         { type:'number' },
  isr:                 { type:'number' }, imss:               { type:'number' },
  infonavit:           { type:'number' }, otras_deducciones:  { type:'number' },
  bonos:               { type:'number' }, horas_extra:        { type:'number' },
  monto_horas_extra:   { type:'number' }, personal_presente:  { type:'number' },
  /* textarea */
  descripcion:          { type:'textarea' }, descripcion_alcance: { type:'textarea' },
  descripcion_trabajo:  { type:'textarea' }, plan_mitigacion:     { type:'textarea' },
  acciones_correctivas: { type:'textarea' }, acciones_tomadas:    { type:'textarea' },
  observaciones:        { type:'textarea' }, notas:               { type:'textarea' },
  testigos:             { type:'textarea' },
};

/* Campos que son FK y se poblan con selects remotos.
   Clave = nombre del campo, valor = id del <select> original en PAGE_SELECTS */
...
async function openEditModal(module, id) {
  ...
  /* Poblar selects remotos dentro del modal y restaurar valor */
  ...
  for (const [tipo, selIds] of Object.entries(cfg)) {
    for (const originalSelId of selIds) {
      ...
      /* Solo poblar si ese campo está en el formulario de edición */
...
function buildEditForm(module, record) {
  ...
  for (const field of fields) {
    ...
    if (remoteSelId) {
      /* Select remoto — se poblará en openEditModal */
      html += `
...
/* ── Filtrar stat-cards del dashboard ──────────────────────────── */
...
/* ── Filtrar botones de accesos rápidos ────────────────────────── */
